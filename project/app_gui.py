...
class IikoReportApp:
    def __init__(self, root):
        ...
        self.end_date = None
        # Экземпляр генератора отчетов с функцией логирования
        self.report_generator = ReportGenerator(log_callback=self.log_message)
        # Загружаем конфигурацию
        self.load_config()
        # Создаем интерфейс
        self.create_widgets()
        # Создаем текстовый виджет для логов
    ...
    def load_config(self):
        ...
        try:
            ...
            self.log_message(f"❌ Ошибка загрузки bases_config.json: {e}")
            self.bases_config = {}
